...
class KGR2baseUploadPreprocessor(BaseLLMBackend, BaseProcessor):
    ...
    def _run(self, args: BaseInterface, ltm: LTM) -> BaseInterface:
        nodes = json.load(open("data/res.json"))["nodes"]
        mapping = {
            "node_name": {"type": "text", "lang": "zh", "index": "bm25"},
            "text_vector": {"type": "vector", "ANN_type": "lsh", "num_dim": 1024},
            "chunk_id": {"type": "text", "lang": "zh"}
        }

        ltm.R2BaseHandler.make_index("kg_test", mapping)
        docs = []
        for node in tqdm(nodes):
            text_vector = self.bge_encode(node["id"])
            doc = {
                "node_name": node["id"],
                "text_vector": text_vector,
                "chunk_id": ",".join([str(x) for x in node["chunk"]])
            }
            docs.append(doc)
        ltm.R2BaseHandler.add_docs("kg_test", docs)
